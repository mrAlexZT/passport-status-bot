...
async def _get_public_proxies_list(
    ...
    for url in sources:
        ...
        for line in text.splitlines():
            ...
            if ":" not in candidate:
                continue
            if candidate.startswith("http://") or candidate.startswith("https://") or candidate.startswith("socks4://") or candidate.startswith("socks5://"):
...
async def _quick_check_proxy(
    ...
    log_info(f"Quick checking {len(proxy_urls)} proxies with timeout {timeout_sec}s, concurrency={max_concurrency}")
...
async def _test_proxy_connection(
    ...
    """
    log_info(f"Testing {len(proxy_urls)} proxies with timeout {timeout_sec}s, concurrency={concurrency}")
    test_url = "https://httpbin.org/ip"
    ...
    async with async_playwright() as p:
        ...
        async def test_proxy(proxy_url: str) -> bool:
            async with sem:
                ...
                page = None
                try:
                    context = await browser.new_context(proxy={"server": proxy_url}, ignore_https_errors=True)
                    ...
                    if response and response.status == 200:
                        try:
                            ...
                            if ip_address:
                                log_info(f"✅ Proxy works: {proxy_url}, IP: {ip_address}")
                                return True
                            else:
                                log_warning(f"⚠️ No IP in response from: {proxy_url}")
                        except Exception as e:
                            log_warning(f"⚠️ Failed to parse JSON from proxy {proxy_url}: {e}")
                    else:
                        status = response.status if response else "no response"
                        log_warning(f"❌ Proxy failed with status {status}: {proxy_url}")
        ...
        results = await asyncio.gather(
            *(test_proxy(proxy) for proxy in proxy_urls),
            return_exceptions=True
        )

        for proxy, result in zip(proxy_urls, results):
    ...
    log_info(f"Found {len(working_proxies)} working proxies from {len(proxy_urls)} tested")
...
async def get_working_proxies():
    ...
    """
    available_proxies = await _get_public_proxies_list(country="ua,pl,de,es,ro,lt,sk", protocol="http,socks4,socks5")
    
    alive_proxies = await _quick_check_proxy(available_proxies)
    
    working_proxies = await _test_proxy_connection(alive_proxies)

    log_info(f"Available proxies: {len(available_proxies)} | Alive proxies: {len(alive_proxies)} | Working proxies: {len(working_proxies)}")
